...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("
        INSERT INTO sow_expenses (sow_id, stage, feed_cost, health_cost, ai_cost, total_cost)
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->execute([$sow_id, $stage, $feed_cost, $health_cost, $ai_cost, $total_cost]);
...
<head>
<meta charset="UTF-8">
<title>Add Expense Record</title>
...
<body class="p-4">
<div class="container">
  <h2 class="mb-4">💰 Add Sow Expense Record</h2>
  <form method="POST">
    <div class="mb-3">
    ...
    <div class="row g-3">
      <div class="col-md-4">
        ...
        <input type="number" step="0.01" id="feed_cost" name="feed_cost" class="form-control" oninput="computeTotal()">
      </div>
      <div class="col-md-4">
        <label>Health Cost (₱)</label>
        <input type="number" step="0.01" id="health_cost" name="health_cost" class="form-control" oninput="computeTotal()">
      </div>
      <div class="col-md-4">
        <label>AI Cost (₱)</label>
        <input type="number" step="0.01" id="ai_cost" name="ai_cost" class="form-control" oninput="computeTotal()">
      </div>
    </div>

    <div class="mt-3 mb-3">
      <label>Total Cost (₱)</label>
      <input type="number" step="0.01" id="total_cost" name="total_cost" class="form-control" readonly>
    </div>
